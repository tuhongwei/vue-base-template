...
const pm = (function () {
  ...
  // 封装一个上报两项核心数据的方法
  pMonitor.logPackage = () => {
    const { url, timeoutUrl, method, timeout } = config;
    const domComplete = pMonitor.getLoadTime();
    const timeoutRes = pMonitor.getTimeoutRes(timeout);
    // 上报页面加载时间
  ...
  pMonitor.init = option => {
    const { url, timeoutUrl, method = 'POST', timeout = 1000 } = option;
    config = { url, timeoutUrl, method, timeout };
    // 绑定事件 用于触发上报数据
    pMonitor.bindEvent();
  };
  return pMonitor;
